            ...
            <form action="{{ route('master.users.store') }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-dropdown name="store_id" :options="['' => 'Pilih Toko..'] + $stores->pluck('name', 'id')->toArray()" :value="old('store_id')" />
                        @error('store_id')
                    ...
                    <div>
                        ...
                        <x-dropdown name="role" :options="collect($roles)->mapWithKeys(fn($r) => [$r => ucfirst($r)])->toArray()" :value="old('role', 'user')" />
                        @error('role')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
